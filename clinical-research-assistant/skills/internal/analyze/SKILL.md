...
**Cross-cutting: Cross-cohort harmonization (per L011).** Whenever an analysis compares two registries or two cohorts drawn from different source systems (SEER vs NCDB, internal cohort vs external cohort, discovery vs validation, etc.), apply the harmonization protocol in `../data-analysis/references/registry-cautions.md` (Cross-Cohort Comparison Harmonization section) BEFORE reporting any side-by-side estimate. Generate a `cohort_harmonization_log.md` artifact documenting inclusion criteria, variable crosswalks, time windows, outcome definitions, and censoring rules applied to make the cohorts comparable. Cite it in Methods. Without this, cross-cohort differences may be entirely definitional artefacts.

**Cross-cutting: Within-recipient PSM and caliper-binding diagnostic (per L039, L040).** When the analysis reports a within-treatment-recipient comparison (e.g., NHB vs NHW *among patients who received surgery*), the estimand is effectiveness, not access — apply the within-recipient PSM protocol in `../data-analysis/references/method-selection-guide.md` (Within-Recipient Propensity Matching section). For every PSM analysis, generate a caliper-sensitivity table across at least three caliper widths (0.10 / 0.20 / 0.25 × SD logit PS) and report whether the caliper is binding per `../data-analysis/references/diagnostics-checklist.md` (Caliper-binding interpretation). State the estimand explicitly in Methods and Results: access HR (full cohort) vs effectiveness HR (within-recipient) must be reported separately when both are scientifically relevant.

**Cross-cutting: Audit-step tagging (per L038).** During pre-submission QC, every change made to the manuscript or analysis output must carry a discoverable audit tag (`[QC-<n>][<category>] <description>`) recorded in `Reports/qc_audit_log_<date>.md`. See `../manuscript-qc/references/checks.md` Check 13 for the tagging schema, log template, and category list. Pair this with comparator-aligned reporting: every OR/HR/RR reported must state its reference category in text, table footnote, and figure caption (see write-methods-results SKILL.md "Comparator-Aligned Reporting — HARD RULE").
